				...
				if(!chkTinhCong_Edit.checked){
					...
					NUT_DS.call({url:url+"rpc/f_nsdongbhxh",data:{y:nam,m:thang,d:ngay}},function(dongbhxh){
						...
						NUT_DS.select({url:url+"thangbangluong_v",where:[["nam","=",nam],["thang","=",thang],["vitrilv","=",prefix]]},function(phucaps){
							NUT_DS.select({url:url+"rpt_monthlysum",where:[["nam","=",nam],["thang","=",thang],["madoitac","=",cboTinhCong_DoiTac.value],(vitrilv=="SM"||vitrilv=="BA"?["vitrilv","=",vitrilv]:["vitrilv","like",vitrilv+"*"]),(_context.user.kvhcs.length?["makhuvuc","in",_context.user.kvhcs]:["thitruong","=",cboTinhCong_ThiTruong.value]),["dulieu","=",dulieu]]},function(res){
													...
													if(rec.manhanvien!=oldMaNhanVien){
														if(oldMaNhanVien){
															...
															var json={
																nam:nam,
																thang:thang,
																madoitac:cboTinhCong_DoiTac.value,
					...
					NUT_DS.select({url:url+"nhansu",select:"idnhansu,manhanvien,sohoso,hoten,quanlytructiep,makhuvuc,diadiemlv",where:[["madoitac","=",cboTinhCong_DoiTac.value],["vitrilv","=",vitrilv]]},function(res){
						var lookupNv={};
						for(var i=0;i<res.length;i++){
							lookupNv[res[i].manhanvien]=res[i];
						}
						NUT_DS.select({url:url+"rpt_bangluong",where:[["nam","=",numTinhCong_Year.value],["thang","=",numTinhCong_Month.value],["madoitac","=",cboTinhCong_DoiTac.value],["vitrilv","=",vitrilv],(_context.user.kvhcs.length?["makhuvuc","in",_context.user.kvhcs]:["thitruong","=",cboTinhCong_ThiTruong.value]),["dulieu","=",dulieu]]},function(res){
							if(res.length){
								var win=window.open(html);
								win.onload=function(){
