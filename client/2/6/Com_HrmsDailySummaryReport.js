	...
	runReport:function(){
		if(numTinhCong_Year.value&&numTinhCong_Month.value){
			...
			NUT_DS.select({url:url+"chucvu_v",where:[["machucvu","in","TF,TL,GDCN"],(_context.user.kvhcs.length?["makhuvuc","in",_context.user.kvhcs]:["thitruong","=",cboTinhCong_ThiTruong.value]),["or",["ngaybatdau","is","null"],["ngaybatdau","<=",date]],["or",["ngayketthuc","is","null"],["ngayketthuc",">=",date]]]},function(cvs){
				...
				NUT_DS.select({url:url+"rpt_sumchitieu",where:[["nam","=",nam],["thang","=",thang],["madoitac","=",cboTinhCong_DoiTac.value],(_context.user.kvhcs.length?["makhuvuc","in",_context.user.kvhcs]:["thitruong","=",cboTinhCong_ThiTruong.value])]},function(chitieu){
					var lookupChiTieu={};
					for(var i=0;i<chitieu.length;i++)lookupChiTieu[chitieu[i].makhuvuc]=chitieu[i];
					NUT_DS.select({url:url+"rpt_dailysummary",where:[["nam","=",nam],["thang","=",thang],["madoitac","=",cboTinhCong_DoiTac.value],(_context.user.kvhcs.length?["makhuvuc","in",_context.user.kvhcs]:["thitruong","=",cboTinhCong_ThiTruong.value]),(vitrilv=="BA"?["vitrilv","=",vitrilv]:["vitrilv","like",vitrilv+"*"]),["dulieu","=",chkTinhCong_Edit.checked?1:0]]},function(res){
						if(res.length){
