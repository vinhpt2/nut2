		...
		NUT_DS.getNow(function(now){
			...
			NUT_DS.select({url:self.urledit+"nhanvien_v",select:"makhuvuc,madoitac",where:["manhanvien","=",_context.user.username]},function(nv){
				if(nv.length){
					...
					NUT_DS.select({url:self.urledit+"chamcong_v",order:"lan",where:[["nam","=",self.nam],["thang","=",self.thang],["ngay","=",self.ngay],["dulieu","=",0],["manhanvien","=","%22"+_context.user.username+"%22"]]},function(res){
						self.lookupData={};
							...
							for(var i=0;i<res.length;i++){
								...
								if(data.thoigianden){
									...
									if(data.thoigianve){
										...
										}else html+='<td><div id="divInOut_SL'+lan+'" class="nut-tile" style="background:white;border:1px solid;color:brown" onclick="Com_HrmsCheckInOut.divInOutSL_onClick(this,'+id+',\''+data.madiemban+'\')"><img src="client/2/6/factory.ico"><br/><b>Check SANLUONG</b></div></td>';
										...
										html+='<td><div id="divInOut_OUT'+lan+'" class="nut-tile" style="background:white;border:1px solid;color:orange" onclick="Com_HrmsCheckInOut.divInOut2_onClick(this,'+id+','+lan+')"><img src="client/2/6/out.ico"><br/><b>Check OUT</b></div></td>';
								} else {
						...
						w2popup.open({
							title: '🚪 <i>Check IN-OUT</i> - <b>Ngày #<i>'+self.nam+'-'+self.thang+'-'+self.ngay+'</i></b>',
							modal:true,
							width: 600,
							height: 450,
							body: html+"</table>",
	...
	divInOut2_onClick:function(elm,id,lan){
		if(lan==1){
			w2popup.message({
				...
				buttons: '<button class="w2ui-btn" onclick="w2popup.message()">Cancel</button><button class="w2ui-btn" onclick="Com_HrmsCheckInOut.doCheckOut('+elm.id+','+id+')">✔️ Ok</button>'
			});
