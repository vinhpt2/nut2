var Com_SysAddLayer={
	run:function(p){
		if(p.parent){
			this.app=p.parent;
			var self=this;
			NUT_DS.select({srl:"sv_appservice_table",where:[["applicationid","=",this.app.applicationid],["tabletype","=","gis"]]},function(tables){
				if(tables.length)
					NUT_DS.select({srl:"syslayer",where:["applicationid","=",self.app.applicationid]},function(res){
						var existLayers={};
	...
	insertLayers:function(layers){
		NUT_DS.insert({srl:"syslayer",data:layers},function(res){
