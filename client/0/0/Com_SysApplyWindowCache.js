var Com_SysApplyWindowCache={
		...
		if(p.records.length){
			...
			w2confirm('Apply change to Window cache?','❓ Confirm',function(evt){
				if(evt=="Yes"){
					...
					else NUT_DS.select({srl:"sv_appservice_table",where:["applicationid","=",self.window.applicationid]},function(tables){
						if(tables.length){
							var lookupTable={};
	...
	cacheTabs:function(lookupTable){
		var self=this;
		NUT_DS.select({srl:"systab",order:["tablevel","orderno"],where:["windowid","=",self.window.windowid]},function(tabs){
			if(tabs.length){
				self.cache.tabs=[];
	...
	cacheFields:function(lookupTable){
		var self=this;
		NUT_DS.select({srl:"sv_field_column",order:"tabid,fieldgroup,orderno",where:["windowid","=",self.window.windowid]},function(fields){
			if(fields.length){
				self.cache.fields=[];
	...
	cacheMenuitems:function(){
		var self=this;
		NUT_DS.select({srl:"sysmenuitem",order:"orderno",where:[["windowid","=",self.window.windowid],["menuitemtype","=","tool"]]},function(menuitems){
			if(menuitems.length){
	...
	updateWindowCache:function(){
		NUT_DS.upsert({srl:"syscache",keys:"windowid,clientid,applicationid",data:{windowid:this.window.windowid,clientid:this.window.clientid,applicationid:this.window.applicationid,config:zipson.stringify(this.cache)}},function(res){
			NUT.tagMsg("Window's cache updated.","lime");
		});
	}
}