var Com_SysApplicationWizard={
	...
	showDlgWizard:function(conf){
		...
		this.a=createWindowTitle(id,divTitle);
		this.a.innerHTML='Wizard.'+ self.app.applicationname;
		this.a.div.innerHTML='<iframe id="'+id+'" style="width:850px;height:550px;border:none;background:white" src="client/0/0/step1.html"></iframe></br><button id="butBack" class="w2ui-btn" onclick="Com_SysApplicationWizard.go(-1)">Close</button> <button id="butNext" class="w2ui-btn" onclick="Com_SysApplicationWizard.go(1)">Next</button>';
	},
	go:function(index){
		var id="divCom_SysApplicationWizard";
		if((index<0&&this.step>1)||(index>0&&this.step<3)){//next-back
			this.step+=index;
			document.getElementById(id).src="client/0/0/step"+this.step+".html";
			...
			var panels=[{ type: 'top', size: '28px', content: '<div id="divTopWz"></div>' },
	...
	designApp:function(){
		...
		//load menu
		NUT_DS.select({srl:"sv_rolemenu_menuitem",where:[["menuitemtype","=","menu"],["applicationid","=",this.app.applicationid]]},function(res){
			var ids={},pids={};
			for(var i=0;i<res.length;i++){
				if(res[i].menuitemid)ids[res[i].menuitemid]=true;
				if(res[i].parentid)pids[res[i].parentid]=true;
			}
			NUT_DS.select({srl:"sysmenuitem",order:"orderno",where:["or",["menuitemid","in",Object.keys(ids)],["menuitemid","in",Object.keys(pids)]]},function(menuitems){
				...
				} else	isGis?$(divTopWz).w2toolbar({name:id,items: nodes,onClick:self.menu_onClick}):$(divLeftWz).w2sidebar({name:id,flatButton:true,nodes: nodes,onClick:self.menu_onClick});
				
				w2ui[id].config=nodes;
			});
		});
		//load window
		NUT_DS.select({srl:"syswindow",order:"windowname",where:[["windowtype","=","window"],["applicationid","=",this.app.applicationid]]},function(res){
	...
	loadTabs:function(div,win){
		...
		var self=this;
		NUT_DS.select({srl:"systab",order:["tablevel","orderno"],where:["windowid","=",win.windowid]},function(tabs){
			if(tabs.length){
