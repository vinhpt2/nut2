var Com_SysWfManager={
	run:function(div){
		this.div=div;
		var self=this;
		NUT_DS.select({srl:"wfjobtype",where:["applicationid","=",_context.curApp.applicationid]},function(res){
			...
			var items=[{type:'html',html:"&nbsp;Workflow: "+str+"</select>"},{type:'break'},{type:'html',html:"&nbsp;User: <select id='cboJobM_User'></select>"},{type:'html',html:"&nbsp;Status: <select id='cboJobM_Status'></select>"},{type:'spacer'},{type:'html',html:"Total: <b id='labJobM_Total'></b> job(s)"}];
			id="divJobM_Top";
	...
	cboJobtype_onChange:function(val){
		if(val){
			NUT_DS.select({srl:"wfjob",where:["jobtypeid","=",val]},function(res){
				if(res.length){
					...
					for(var i=0;i<res.length;i++){
						...
						bar.onclick=function(){
							var self=this;
							NUT_DS.select({srl:"sv_tab_table",where:["tablename","=","wfjob"]},function(res){
								if(res.length){
									...
									if(conf){
										...
										NUT_DS.select({srl:"syscache",where:["windowid","=",tag]},function(res){
											if(res.length){
						...
						a.onclick=function(){
							if(this.div){
								...
								NUT_DS.select({srl:"wfhistory",where:["jobid","=",this.tag.jobid]},function(res){
									for(var i=0;i<res.length;i++){
										var rec=res[i];
