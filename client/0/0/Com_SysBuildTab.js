var Com_SysBuildTab={
	...
	run:function(p){
		if(p.records.length){
			...
			var self=this;
			NUT_DS.select({srl:"sv_appservice_table",order:"servicename,tabletype,tablename",where:["applicationid","=",this.window.applicationid]},function(tables){
				if(tables.length)
					NUT_DS.select({srl:"systab",where:["windowid","=",self.window.windowid]},function(res){
						var existTabs={};
	...
	showDlgBuild:function(tables, existTabs){
		...
		for(var i=0;i<tables.length;i++){
			var table=tables[i];
			var label=table.tablename+" ("+table.servicename+")";
			if(isBlank)items.push({id:table.tableid,text:label});
			lookupTable[table.tableid]=table;
			var fld={field:table.tableid,type:'checkbox',html:{label:label,column:i%2,attr:lookup[table.tableid]?"disabled":"tabindex=0"}};
			fields.push(fld);
		}
	...
	addMissField:function(tab){
		var self=this;
		NUT_DS.select({srl:"sysfield",select:"columnid",where:["tabid","=",tab.tabid]},function(res){
			var columnids=[];
			for(var i=0;i<res.length;i++)columnids.push(res[i].columnid);
			NUT_DS.select({srl:"syscolumn",where:[["tableid","=",tab.tableid],["columnid","!in",columnids]]},function(columns){
				if(columns.length){
					var fields=[];
	...
	insertTab:function(table,orderno,parenttab,callback){
		...
		var self=this;
		NUT_DS.select({srl:"syscolumn",order:"columnid",where:["tableid","=",table.tableid]},function(columns){
			if(columns.length){
				var fields=[];
				for(var i=0;i<columns.length;i++){
					var col=columns[i];
				...
				NUT_DS.insert({srl:"systab",data:tab},function(res){
					if(res.length){
	...
	insertFields:function(fields){
		NUT_DS.insert({srl:"sysfield",data:fields},function(res){
			if(res.length)NUT.tagMsg("Records inserted.","lime",document.activeElement);
		});
	}
}