var Com_SysImportTable={
	run:function(p){
		if(p.records.length){
			...
			switch(this.service.servicetype){
				case "postgrest":
					NUT_DS.call({srl:"rpc/f_pgrst_watch"},function(){//reload schema
						NUT_DS.queryMetadata(self.service.urledit,function(metadata){
							...
							if(definitions){
								...
								NUT_DS.select({srl:"systable",where:["serviceid","=",self.service.serviceid]},function(res){
									var lookup={};
	...
	insertTable:function(name,definition){
		...
		var self=this;
		NUT_DS.insert({srl:"systable",data:table},function(res){
			if(res.length){
	...
	insertColumns:function(columns){
		NUT_DS.insert({srl:"syscolumn",data:columns},function(res){
			NUT.tagMsg("Record inserted.","lime",document.activeElement);
		});
	}
}