var Com_SysCacheDomain={
	run:function(p){
		if(p.records.length){
			...
			var self=this;
			NUT_DS.select({srl:"systable",where:[["serviceid","=",this.service.serviceid],["iscache","=",true]]},function(res){
				if(res.length){
					lookup={};
	...
	cacheDomain:function(table){
		NUT_DS.select({srl:table.tablename,select:[table.columnkey,table.columndisplay]},function(res){
			var domains=[];
			...
			NUT_DS.update({srl:"sysdomain",where:["tableid","=",table.tableid],data:data},function(res){
