var Com_SysWfBackJob={
	run:function(p){
		if(p.records.length){
			...
			NUT_DS.select({srl:"wfjobtype",select:"users",where:["jobtypeid","=",this.job.jobtypeid]},function(res){
				if(res.length){
	...
	updateJob(){
		NUT_DS.update({srl:"wfjob",where:["jobid","=",this.job.jobid],data:{sentfrom:_context.user.username,assignedto:cboJob_User.value,assignstatusid:null,note:txtJob_Note.value}},function(res){
