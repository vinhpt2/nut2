var Com_SysWfRunJob={
	run:function(p){
		if(p.records.length){
			...
			else NUT_DS.select({srl:"wfjobtype",select:"jobtypedata,users",clientid:_context.curApp.clientid,where:["jobtypeid","=",this.job.jobtypeid]},function(res){
				if(res.length){
	...
	showWorkflow:function(wfconf){
		...
		if(conf){
			...
			NUT_DS.select({srl:"syscache",clientid:_context.curApp.clientid,where:["windowid","=",this.job.windowid]},function(res){
				if(res.length){
		...
		w2popup.open({
			...
			onOpen:function(evt){
				evt.onComplete=function(){
					var a=createWindowTitle(conf.windowid,divJob_Title,true);
					...
					var panels=[{ type: 'top', size: '26px', content: '<div id="divJob_Top" class="nut-full"></div>' },
	...
	send:function(isFinish){
		...
		NUT_DS.update({srl:"wfjob",where:["jobid","=",job.jobid],data:data},function(){
			...
			NUT_DS.insert({srl:"wfhistory",data:data});
