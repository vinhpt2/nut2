...
function body_onLoad(){
	...
	NUT_DS.select({srl:"sysapplication",order:"orderno"},function(apps){
		...
		NUT_DS.select({srl:"sysuser",where:["clientid","=",user.clientid]},function(users){
			var userItems=[];
			for(var i=0;i<users.length;i++)userItems.push(users[i].username);
			
			NUT_DS.select({srl:"systable",where:["clientid","=",user.clientid]},function(tables){
				var tableItems=[];
						...
						"✔️ Save": function (evt) {
							...
							if(_bpmnModeler.get("elementRegistry").get(startStep)){
								...
								_bpmnModeler.saveXML().then(function(xml){
									...
									self.record.clientid=user.clientid;
...
function loadMenu(){
	NUT_DS.select({srl:"wfjobtype",order:"jobtypename",where:["clientid","=",_context.user.clientid]},function(res){
		var nodes=[], lookup={};			
...
function butUsers_onClick(input){
	NUT_DS.select({srl:"sysgroup",order:"groupname",where:["clientid","=",_context.user.clientid]},function(groups){
		var grpLookup={};
		for(var i=0;i<groups.length;i++){
			var group=groups[i];
			grpLookup[group.groupid]=group.groupname;
		}
		NUT_DS.select({srl:"sysuser",where:["clientid","=",_context.user.clientid]},function(res){
			var fields=[];
			for(var i=0;i<res.length;i++){
				var user=res[i];
