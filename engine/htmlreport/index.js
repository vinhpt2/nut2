...
function loadMenu(){
	NUT_DS.select({srl:"syswindow",order:"windowname",where:[["clientid","=",_context.user.clientid],["windowtype","=","report"]]},function(res){
		var nodes=[], lookup={};			
...
function tool_onClick(evt){
	...
	switch(item.id){
		...
		case "detail":
			...
			break;
		case "CONN":
			NUT_DS.select({srl:"sysservice",order:"servicename",where:["clientid","=",_context.user.clientid]},function(services){
						...
						if(w2ui[id]){
							...
							actions: {
								...
								"✔️ Ok": function (evt) {
									...
									this.record.clientid=_context.user.clientid;
									...
									if(_record){//update report
										...
										NUT_DS.insert({srl:"syswindow",data:this.record},function(res){
											if(res.length){
												_record=res[0];
												NUT_DS.insert({srl:"syscache",data:{clientid:_record.clientid,applicationid:_record.applicationid,windowid:_record.windowid,config:zipson.stringify(_report)}},function(res){
